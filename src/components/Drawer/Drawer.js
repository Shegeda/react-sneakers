...
const Drawer = ({ onClose, onRemove, items = [], opened }) => {
  ...
  const onClickOrder = async () => {
    try {
      setIsLoading(true)
      const { data } = await axios.post('https://61a3f974d5e8330017292209.mockapi.io/react-sneakers/orders', {
        items: cartItems
      })
      setOrderId(data.id)
      ...
      for (let i = 0; i < cartItems.length; i++) {
        ...
        await axios.delete('https://61a3f974d5e8330017292209.mockapi.io/react-sneakers/cart/' + item.id)
        await delay(1000)
      }
